...
from botcore.features.twitch_state import reload_twitch_notifications_from_disk, twitch_notifications, update_twitch_notification_state
...
async def run_twitch_live_check():
    ...
    for guild_key, guild_notifications in twitch_notifications.items():
        if not guild_key.isdigit() or not isinstance(guild_notifications, dict):
            continue
        for notification_key, notification in guild_notifications.items():
            if isinstance(notification, dict):
                twitch_login = notification.get("twitch_login")
                if isinstance(twitch_login, str) and twitch_login:
