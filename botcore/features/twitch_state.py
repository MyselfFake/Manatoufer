...
def upsert_twitch_notification(
    ...
    guild_notifications[twitch_login] = {
...
def toggle_twitch_notification(
    ...
    guild_key = str(guild_id)
    guild_notifications = twitch_notifications.setdefault(guild_key, {})
    existing = guild_notifications.get(twitch_login)
    if isinstance(existing, dict) and existing.get("channel_id") == channel_id and existing.get("role_id") == role_id:
        guild_notifications.pop(twitch_login, None)
...
def update_twitch_notification_state(
    ...
    notification = guild_notifications.get(twitch_login)
    if notification is None:
        return
    notification["is_live"] = is_live
    notification["last_stream_id"] = last_stream_id
    guild_notifications[twitch_login] = notification
    save_twitch_notifications_to_disk(twitch_notifications)
